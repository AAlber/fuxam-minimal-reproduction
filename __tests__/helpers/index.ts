...
import type { ContentBlockSpecsMapping } from "@/src/app/types/content-blocks/specs.types";
...
export const createMockUserRole = (
  args: Partial<PrismaUserRole & { role: RoleDeprecated }>,
): PrismaUserRole => {
  const {
    id = `role-${generateUid()}`,
    institutionId = `instiRole-${generateUid()}`,
    layerId = `layerRole-${generateUid()}`,
    role = "admin" as RoleDeprecated,
