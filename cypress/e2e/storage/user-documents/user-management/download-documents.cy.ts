...
describe("Download documents from user management page", () => {
  beforeEach(() => {
  ...
  it("should upload and download documents successfully", () => {
    // Click upload button
    ...
    cy.get('[data-cy="user-management-user-documents-download-button"]').first().click();

    // Verify download was initiated
    cy.readFile('cypress/downloads/example.json').should('exist');

    // Click download button for second file
    cy.get('[data-cy="user-management-user-documents-download-button"]').last().click();

    // Verify second download was initiated
    cy.readFile('cypress/downloads/test1.txt').should('exist');
  });
});
